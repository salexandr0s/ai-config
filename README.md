...
The installer creates symlinks from each tool's config directory into this repo, creates local overlay templates when missing, and renders the live Codex config. Existing files are backed up with a timestamp suffix. Re-running is safe and idempotent.
